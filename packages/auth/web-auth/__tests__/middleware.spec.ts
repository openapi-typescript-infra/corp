...
type IdentityInternalApi = ReturnType<typeof createClient<Pick<IdentityInternal, '/identity/access'>>>;
...
describe('middleware', () => {
  test('disabled', async () => {
    expect('We need to disable this test temporarily to pivot out of some cyclic dependencies').toBeTruthy();
  });
  ...
  test('authorization', async () => {
    ...
    if (app.locals.datasources.identityInternal) {
      vi.spyOn(app.locals.datasources.identityInternal as unknown as IdentityInternalApi, 'POST').mockImplementation(
        async (method, args) => {
          if (method !== '/identity/access') {
            throw new Error(`Unexpected method: ${method}`);
          }
          return Promise.resolve({
            data: args.body.subjects?.[0]?.identifier === '37911d60-d5d1-43af-8413-fd3174d53477' ? {
              grants: [{ identifier: '37911d60-d5d1-43af-8413-fd3174d53477', namespace: 'patient-uuid' }]
            } : { grants: [] },
            response: {
              status: 200
            },
