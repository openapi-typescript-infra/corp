/* eslint-disable */
// The below is from https://github.com/bruth/jsonpatch-js/blob/9a1874ec9bc694029f18bd7799a307c92305e7ca/jsonpatch.js with modifications:
//  1. Allow 'inc' operation to increment a value
//  2. Add 'test' operation to look for a value and stop the patch if not there
//  3. Add 'clear' operation to remove a value by name (objects only)
//  4. Add 'merge' operation to merge an object into another (objects only)
...
(function () {
  ...
  (function (root, factory) {
      ...
      if (className === '[object Array]') {
        ...
        if (result) {
          while (size--) {
            if (
              !(result =
                __indexOf.call(a, size) >= 0 === __indexOf.call(b, size) >= 0 &&
                _isEqual(a[size], b[size], stack))
            ) {
              break;
            }
          }
        ...
        if (
          __indexOf.call(a, 'constructor') >= 0 !== __indexOf.call(b, 'constructor') >= 0 ||
    ...
    JSONPointer = (function () {
      ...
      JSONPointer.prototype.getReference = function (parent, noFail) {
        ...
        for (_i = 0, _len = _ref.length; _i < _len; _i++) {
          ...
          if (!(step in parent)) {
            // Added by maxm to allow for noFail from ClearPatch
            if (noFail) {
              return undefined;
            }
            throw new PatchConflictError(
              'Array location out of bounds or not an instance property',
            );
          }
          parent = parent[step];
        }
    ...
    // Max M additions
    IncrementPatch = (function (_super) {
      __extends(IncrementPatch, _super);

      function IncrementPatch() {
        return IncrementPatch.__super__.constructor.apply(this, arguments);
      }

      IncrementPatch.prototype.validate = function (patch) {
        if (!('value' in patch)) {
          throw new InvalidPatchError('Increment requires a numeric value');
        }
      };

      IncrementPatch.prototype.apply = function (document) {
        var accessor, reference, value;
        reference = this.path.getReference(document);
        accessor = this.path.accessor;
        value = this.patch.value;
        const result = Number((reference[accessor] || 0) + value);
        if (
          ...
          throw new InvalidPatchError('Resulting value is not a number');
        }
        reference[accessor] = result;
        return document;
      };

      return IncrementPatch;
    })(JSONPatch);
    ClearPatch = (function (_super) {
      __extends(ClearPatch, _super);

      function ClearPatch() {
        return ClearPatch.__super__.constructor.apply(this, arguments);
      }

      ClearPatch.prototype.apply = function (document) {
        var accessor, reference, value;
        reference = this.path.getReference(document, true);
        if (!reference) {
          // We did our job.
          return document;
        }
    ...
    MergePatch = (function (_super) {
      __extends(MergePatch, _super);

      function deepMerge(a, b) {
        if (Array.isArray(a) && Array.isArray(b)) {
          return [...a, ...b];
        } else if (typeof a === 'object' && a !== null && typeof b === 'object' && b !== null) {
          const merged = { ...a }; // Start with a copy of `a`
          for (const k in b) {
            if (b.hasOwnProperty(k)) {
              merged[k] = b[k] && typeof b[k] === 'object' ? deepMerge(merged[k], b[k]) : b[k];
            }
          }
          return merged;
        } else {
      ...
      function MergePatch() {
        return MergePatch.__super__.constructor.apply(this, arguments);
      }

      MergePatch.prototype.apply = function (document) {
        var accessor, reference, value;
        reference = this.path.getReference(document);

        if (typeof reference !== 'object') {
          throw new Error('An object is required for merging');
        }
        return deepMerge(reference, this.patch.value);
      };

      return MergePatch;
    })(JSONPatch);
    PushPatch = (function (_super) {
      __extends(PushPatch, _super);

      function PushPatch() {
        return PushPatch.__super__.constructor.apply(this, arguments);
      }

      PushPatch.prototype.validate = function (patch) {
        if (!('value' in patch)) {
          throw new InvalidPatchError();
        }
      };

      PushPatch.prototype.apply = function (document) {
        var accessor, reference, value;
        reference = this.path.getReference(document);
        accessor = this.path.accessor;
        value = this.patch.value;
        if (isArray(reference)) {
          ...
          if (!isArray(reference[accessor])) {
            reference[accessor] = [value];
          } else {
            reference[accessor].push(value);
          }
